...
with open('kb.json','r',encoding='utf-8') as f:
    for line in f:
        ...
        minor_abstract = [predictor['object'] for predictor in data if predictor['predicate'] not in ['摘要', '义项描述']]
        ...
        other_abstract = ""
        if len(minor_abstract):
            for ab in minor_abstract:
                other_abstract += ab

        alias = item['alias']
        collect.append({'id':word_id,'word':word, 'type':type, 'abstract': core_abstract, 'fulltext': other_abstract})

        for alias_ in alias:
            collect.append({'id': word_id, 'word': alias_, 'type':type, 'abstract': core_abstract, 'fulltext': other_abstract})
        count +=1
        if count % 100 == 0:
            print('.')
