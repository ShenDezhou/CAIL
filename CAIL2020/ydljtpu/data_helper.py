...
class DataHelper:
    ...
    # 拼接feature文件的路径
    def get_feature_file(self, tag):   # 跳来跳去的，写的什么几把
    ...
    def __get_or_load__(self, name, file):
        if getattr(self, name) is None:   # 如果这个属性没有的话就读取，有的话就直接返回
            with self.get_pickle_file(file) as fin:
                print('loading', file)
    ...
    @property
    def dev_loader(self):
        return self.DataIterator(*self.load_dev(),   # 这个dataiterator不是pytorch里那个，是作者自己实现的一个
    ...
    def train_loader(self):
        return self.DataIterator(*self.load_train(),   # example, feature, graph
                                 ...
                                 device=xm.xla_device(),   # 读取来的数据放gpu里？不好吧？
