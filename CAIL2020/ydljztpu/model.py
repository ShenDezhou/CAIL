...
class BertSupportNetX(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.fc_hidden_size = config.num_fc_hidden_size
    ...
    def forward(self, doc_input_ids, doc_input_mask, doc_segment_ids,
           query_mapping, start_mapping, all_mapping,tok_to_orig_index,
            ids, y1, y2, q_type, is_support):
        # doc_ids, doc_mask, segment_ids = batch['context_idxs'], batch['context_mask'], batch['segment_idxs']
        # roberta不可以输入token_type_ids
        context_encoding = self.encoder(input_ids=doc_input_ids,
                                        attention_mask=doc_input_mask,
                                        token_type_ids=doc_segment_ids#可以注释
                                              )[0]
        # batch['context_encoding'] = all_doc_encoder_layers
        ...
        start_logits = self.start_linear(input_state).squeeze(2) - 1e30 * (1 - doc_input_mask)
        end_logits = self.end_linear(input_state).squeeze(2) - 1e30 * (1 - doc_input_mask)
