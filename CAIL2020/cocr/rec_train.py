...
def train(net, optimizer, scheduler, loss_func, train_loader, eval_loader, to_use_device,
    ...
    logger.info(f'train dataset has {len(train_loader.dataset)} samples, {all_step} in dataloader')
    logger.info(f'eval dataset has {len(eval_loader.dataset)} samples, {len(eval_loader)} in dataloader')
...
def main():
    ...
    # cfg = parse_args()
    parser = argparse.ArgumentParser(description='train')
    parser.add_argument('-c', '--config', type=str, default='config/rec.json', help='train config file path')
    args = parser.parse_args()
