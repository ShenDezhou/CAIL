...
def build_loader(dataset, config):
    ...
    """
    config_dict = config.__dict__
    dataloader_type = config_dict.pop('type')
    assert dataloader_type in support_loader, f'{dataloader_type} is not developed yet!, only {support_loader} are support now'

    # build collate_fn
    if 'collate_fn' in config_dict:
        config.collate_fn.dataset = dataset
        collate_fn = build_collate_fn(config_dict.pop('collate_fn'))
    else:
        collate_fn = None
    dataloader_class = eval(dataloader_type)(dataset=dataset, collate_fn=collate_fn, **config_dict)
