...
config.train_options = {
    ...
    'checkpoint_save_dir': f"./model/{config.exp_name}/checkpoint", # 模型保存地址，log文件也保存在这里
    'device': 'cuda:0',# 不建议修改
    'epochs': 1,
    'fine_tune_stage': ['backbone', 'neck', 'head'],
    'print_interval': 10,  # step为单位
    'val_interval': 50,  # step为单位
...
config.dataset = {
    ...
    'train': {
        'dataset': {
            ...
            'file': r'F:\CAIL\CAIL2020\cocr\data\icdar2015\recognition\train.txt',
            'input_h': 32,
            'mean': 0.5,
            'std': 0.5,
            'augmentation': False,
    ...
    'eval': {
        'dataset': {
            'type': 'RecTextLineDataset',
            'file': r'F:\CAIL\CAIL2020\cocr\data\icdar2015\recognition\test.txt',
            'input_h': 32,
            'mean': 0.5,
            'std': 0.5,
            'augmentation': False,
