...
from lawrouge import FoldersRouge

prefix_path = "result\\dev\\"
...
if __name__ == "__main__":
    ...
    #
    # os.chdir('..')
    get_path(os.path.join(prefix_path, "prediction"))
    pred_list = []
    with open(input_path.replace("../",""),'r', encoding='utf8') as fr:
        for line in fr:
            item = json.loads(line)
            pred_list.append(os.path.join(prefix_path, "prediction", item['id'] + ".txt"))
            # with open(os.path.join(prefix_path, "prediction", item['id']+".txt"),'w', encoding='utf8') as fw:
            #     lines = item['text']
            #     lines = [line['sentence'] for line in lines]
            #     fw.write(getSummary("\n".join(lines)))
    print(pred_list)

    get_path(os.path.join(prefix_path, "gold"))
    gold_list = []
    with open(input_path.replace("../",""),'r', encoding='utf8') as fr:
        for line in fr:
            item = json.loads(line)
    ...
    files_rouge = FoldersRouge()
    scores = files_rouge.get_scores(pred_list, gold_list, avg=True)
    print(scores)
    print('weighted F1-score:', 0.2*scores['rouge-1']['f'] + 0.4*scores['rouge-2']['f']+ 0.4*scores['rouge-l']['f'])
