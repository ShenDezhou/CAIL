...
class Data:
    ...
    def _load_file(self, filename, train: bool = True):
        ...
        class_dic = ['买卖合同', '承包合同', '技术合同', '知识产权合同', '其他类合同', '房地产合同', '代理合同', '居间合同', '转让合同', '贸易合同', '劳动合同', '托管合同', '仓储合同', '服务合同', '经营合同', '储运合同', '租赁合同', '其他合同', '运输合同', '承揽合同', '借款合同', '供用合同', '金融合同', '房产合同']
        sc_list, bc_list, label_list = [], [], []
        for row in data_frame.itertuples(index=False):
            ...
            if train:
                ...
                label_list.append(class_dic.index(row[0]))
            else:  # test
                sc_list.append(sc_tokens)
                bc_list.append(bc_tokens)
