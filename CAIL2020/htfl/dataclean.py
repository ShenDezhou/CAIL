...
classy_dic = ['买卖合同', '承包合同', '技术合同', '知识产权合同', '其他类合同', '房地产合同', '代理合同', '转让合同', '贸易合同', '劳动合同', '服务合同', '经营合同', '租赁合同', '运输合同', '承揽合同', '借款合同', '金融合同']
#10387
def sample():
    df = pandas.read_csv("dataset/train-phase2.csv")
    ...
    dfsample = []
    for type in classy_dic:
        dfx = df.loc[df['type1'] == type]
        if type=='房地产合同':
            dfx = df.loc[(df['type1'] == '房产合同') |(df['type1'] == '房地产合同')]
        elif type=='其他类合同':
            dfx = df.loc[(df['type1'] == '其他合同') | (df['type1'] == '其他类合同')]


        print(type,len(dfx))
        dfx = dfx.loc[dfx['enpercent']] # remove English articles.
        dfy = dfx.loc[dfx['content'].map(len) >= 50]
        dfz = dfy.sample(n=611, replace=True).copy()
        dfsample.append(dfz)

    print(len(dfsample))
    finaldf = pandas.concat(dfsample)
    finaldf.to_csv("dataset/train-phase3.csv", columns=['type1','title','content'], index=False)
