...
class BiDAFQA(nn.Module):
    def __init__(self, config, gpu_list, *args, **params):
        ...
        self.rank_module = nn.Linear(self.hidden_size * 2 * config.getint("data", "topk"), 1)
    ...
    def forward(self, data, config, gpu_list, acc_result, mode):
        ...
        context = context.view(batch * option * k, -1)
        question = question.view(batch, option, 1, -1).repeat(1, 1, k, 1)
        question = question.view(batch * option * k, -1)
